...
- File is streamed to analysis servers; held in memory only, deleted after analysis (hard cap 36 h)
...
**Acceptance Criteria**:
- Each flagged issue includes a natural-language explanation generated by Gemini 2.5 Flash
...
- For "missing reference" errors, system suggests the reference entry format if the source can be found via Crossref
...
**Priority**: V1.1 | **Points**: 3 | **Status**: Roadmap — not in MVP
...
**Acceptance Criteria**:
- System runs verification pipeline (Crossref → DOI resolution) for each reference
- References not found are flagged for AI plausibility scoring
- Gemini 2.5 Flash assesses: Does the journal exist? Is the author a real researcher? Is the volume/issue/page format plausible?
...
**Acceptance Criteria**:
- System checks each reference against Crossref retraction metadata (`is-retracted-by`)
...
**Acceptance Criteria**:
- PayPal Checkout flow for plan selection and payment
- Supports PayPal wallet and linked payment methods
- Monthly and annual billing options (annual = 2 months free)
- Subscription activates immediately upon successful payment
- Confirmation email with receipt is sent via PayPal
...
- Billing history shows all past invoices (via PayPal account)
...
- Admin can set document retention period (default 36 hours, configurable up to 30 days)
...
> **Status: Deferred — post-MVP.** The MVP is anonymous by design (ADR-001): no registration, no OAuth, no profiles. Stories in this epic remain valid product intent for the accounts/institutional tier and are tracked as roadmap backlog, not MVP scope.

### US-080: Register with Email
**Priority**: V2 (post-MVP) | **Points**: 3
