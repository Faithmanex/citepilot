...
export default function MatchingPanel({ data }: MatchingPanelProps) {
  ...
  const summaryCards = [
    ...
    { label: "Spelling Mismatches", value: spellingMismatches, sub: "Author name discrepancies", icon: Shuffle, color: "#1E3A8A", bg: "#DBEAFE" },
    { label: "Year Mismatches", value: yearMismatches, sub: "Year differs from Crossref", icon: CalendarX2, color: "#6D28D9", bg: "#EDE9FE" },
  ];
    ...
    <section className="space-y-5 animate-fade-in" id="panel-matching">
      <div className="bg-[#FAF6EC] border border-[#C7BC9F] rounded-2xl p-5">
        ...
        <p className="text-xs text-[#696050]">
          Bidirectional matching between in-text citations and reference list entries, with author spelling and year discrepancy detection.
        </p>
      </div>
